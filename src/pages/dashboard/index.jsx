...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [m, e, c, a, p, u, pm] = await Promise.all(queries);
      setStats({ members: m.count, events: e.count, certs: c.count, approved: a.count, pending: p.count, upcoming: u.count, pendingMembers: pm.count });

      if (['leader', 'operations', 'social_media', 'promotions'].includes(role)) {
        const { data } = await supabase
          .from('certifications')
          .select('name, cert_title, status, created_at, department')
            ...
            {recent.map((c, i) => (
              <div key={i} className="flex items-center justify-between px-5 py-3"
                ...
                <div>
                  ...
                  <p className="font-mono" style={{ fontSize: '9px', color: 'var(--text-muted)' }}>{c.cert_title} · {c.department}</p>
                </div>
