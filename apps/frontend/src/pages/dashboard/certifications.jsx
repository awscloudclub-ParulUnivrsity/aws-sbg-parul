...
We're thrilled to announce that ${c.name} from ${c.department || 'Parul University'} has successfully earned the ${c.certificate_title} certification! 🏅☁️
...
function CertCard({ c, onStatusChange }) {
  ...
  const updateStatus = async (status) => {
    setSaving(true);
    try {
      await api.updateCertification(c.id, { status, post_draft: draft });
    } catch (err) {
      console.error(err);
    } finally {
      setSaving(false);
