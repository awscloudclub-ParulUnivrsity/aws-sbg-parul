...
export default function TeamManagePage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error('team_members load error:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const add = async () => {
    ...
    try {
      ...
      const found = profiles.find(p => p.email.toLowerCase() === form.email.trim().toLowerCase());
      if (!found) { setErr('No account found with that email. Ask them to sign up first.'); setSaving(false); return; }

      // Check if already in team
      const existing = team.find(t => t.profile_id === found.id);
      ...
      await api.updateProfile(found.id, { approved: true, department: form.department });
    } catch (error) {
